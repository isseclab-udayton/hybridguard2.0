...
(function($){
	$.fn.boutique = function(options){
		...
		var o = $.extend({
			...
			front_img_height:			400,				// Height of the frontal image (value in pixels / 'auto')
		...
		$.each(this, function(){
			var $container = $(this).addClass('boutique').show(),
				$parent = $container.parent(),
				$lis = $container.children('li');

			// Set constants
			var autotimer, $newitem1, $newitem2, $newitem3, $newitem4, $newitem5, next, iegrow, container_height,
			...
			if(o.front_img_height === 'auto'){
				var_heights = true;
				o.front_img_height = 0;
			}

			// Set order id's
			if(items === 1){
				$li[1].clone().addClass('frame1').prependTo($container).data('framesource', 1);
				$li[1].clone().addClass('frame2').prependTo($container).data('framesource', 1);
			}
			else if(o.starter === 2){
				$li[1].clone().addClass('frame2').prependTo($container).data('framesource', 1);
				$li[items].clone().addClass('frame1').prependTo($container).data('framesource', items);
			}
			else if(o.starter === 1){
				$li[items-1].clone().addClass('frame1').prependTo($container).data('framesource', items-1);
				$li[items].clone().addClass('frame2').prependTo($container).data('framesource', items);
			}
			else{
				$li[o.starter-1].clone().addClass('frame2').prependTo($container).data('framesource', o.starter-1);
				$li[o.starter-2].clone().addClass('frame1').prependTo($container).data('framesource', o.starter-2);
			}
			$li[o.starter].clone().addClass('frame3').prependTo($container).data('framesource', o.starter);
			if(items===1){
				$li[1].clone().addClass('frame4').prependTo($container).data('framesource', 1);
				$li[1].clone().addClass('frame5').prependTo($container).data('framesource', 1);
			}
			else if(o.starter === (items-1)){
				$li[items].clone().addClass('frame4').prependTo($container).data('framesource', items);
				$li[1].clone().addClass('frame5').prependTo($container).data('framesource', 1);
			}
			else if(o.starter === items){
				$li[1].clone().addClass('frame4').prependTo($container).data('framesource', 1);
				$li[2].clone().addClass('frame5').prependTo($container).data('framesource', 2);
			}
			else{
				$li[o.starter+1].clone().addClass('frame4').prependTo($container).data('framesource', o.starter+1);
				$li[o.starter+2].clone().addClass('frame5').prependTo($container).data('framesource', o.starter+2);
			}

			// Cache the first 5
			var $item1 = $container.children('.frame1'),
				$item2 = $container.children('.frame2'),
				$item3 = $container.children('.frame3'),
				$item4 = $container.children('.frame4'),
				$item5 = $container.children('.frame5');
			...
			var li_border			= parseInt($lis.css('borderLeftWidth'), 10),
				...
				front_height		= Math.round(o.front_img_height + front_margin.top + front_margin.bottom + (li_padding*2) + (li_border*2)),
			...
			$item1.css({ left:0, top:back_top }).css({ opacity:o.back_opacity })
				...
				.children('h6:visible').css({ fontSize:back_header });
			$item2.css({ left:item2_pos, top:behind_top, zIndex:2 }).css({ opacity:o.behind_opacity })
				...
				.children('h6:visible').css({ fontSize:behind_header });
			$item3.css({ left:item3_pos, top:front_top, zIndex:3 }).css({ opacity:1 })
				.find('a *').css({ cursor:'pointer' }).end()
				.find('img').css({ width:o.front_img_width, height:o.front_img_height, marginTop:front_margin.top, marginRight:front_margin.right, marginBottom:front_margin.bottom, marginLeft:front_margin.left, opacity:1 })
				.siblings('span:visible').css(front_span_animate)
				.children('h6:visible').css({fontSize:front_header});
			...
			$item5.css({ left:item5_pos, top:back_top }).css({ opacity:o.back_opacity })
				...
				.children('h6:visible').css({fontSize:back_header});
			if(ie6){
				$back.find('span:visible').css(ie6_back_span_animate);
				$behind.find('span:visible').css(ie6_behind_span_animate);
				$front.find('span:visible').css(ie6_front_span_animate);
			}
...
// FUNCTIONS
			...
			function moveRight(times, state){
				...
				if(typeof pre_move_callback === 'function'){
					...
					pre_move_callback(href, containerid);
				}
				...
				$newitem1 = $item2;
				$newitem1.removeClass('frame2').addClass('frame1').stop().animate({ opacity:o.back_opacity, left:0, top:back_top }, zpeed, eazing)
					.find('img').stop().animate({ width:back_img_width, height:back_img_height, marginTop:back_margin.top, marginRight:back_margin.right, marginBottom:back_margin.bottom, marginLeft:back_margin.left, opacity:1 }, zpeed, eazing)
					.end().find('h6:visible').stop().animate({ fontSize:back_header }, zpeed, eazing);
				...
				$newitem2 = $item3;
				$newitem2.removeClass('frame3').addClass('frame2').stop().animate({ opacity:o.behind_opacity, left:item2_pos, top:behind_top }, zpeed, eazing)
					.find('img').stop().animate({ width:behind_img_width, height:behind_img_height, marginTop:behind_margin.top, marginRight:behind_margin.right, marginBottom:behind_margin.bottom, marginLeft:behind_margin.left, opacity:1 }, zpeed, eazing)
					.end().find('h6:visible').stop().animate({ fontSize:behind_header }, zpeed, eazing);
				...
				$newitem3.removeClass('frame4').addClass('frame3').stop().animate({ opacity:1, left:item3_pos, top:front_top }, zpeed, eazing)
					.find('img').stop().animate({ width:o.front_img_width, height:o.front_img_height, marginTop:front_margin.top, marginRight:front_margin.right, marginBottom:front_margin.bottom, marginLeft:front_margin.left, opacity:1 }, zpeed, eazing)
					.end().find('h6:visible').stop().animate({ fontSize:front_header }, zpeed, eazing);
				...
				$newitem4 = $item5;
				$newitem4.removeClass('frame5').addClass('frame4').stop().animate({ opacity:o.behind_opacity, left:item4_pos, top:behind_top }, zpeed, eazing)
					.find('img').stop().animate({width:behind_img_width, height:behind_img_height, marginTop:behind_margin.top, marginRight:behind_margin.right, marginBottom:behind_margin.bottom, marginLeft:behind_margin.left, opacity:1 }, zpeed, eazing)
					.end().find('h6:visible').stop().animate({ fontSize:behind_header }, zpeed, eazing);
					...
					.animate({ opacity:o.back_opacity }, zpeed, function(){
						...
						if(!$newitem3.is(':animated')){
							...
							if(typeof move_callback === 'function'){
								...
								move_callback(href, containerid);
							}
						}
					})
					.find('img').css({ width:back_img_width, height:back_img_height, marginTop:back_margin.top, marginRight:back_margin.right, marginBottom:back_margin.bottom, marginLeft:back_margin.left, opacity:1 })
					.end().find('h6:visible').css({ fontSize:back_header })
				...
				if(ie6){ $item5.find('span:visible').css(ie6_back_span_animate); }
				else{ $item5.find('span').css(back_span_animate); }
				...
				if(var_heights && o.variable_container_height){
			...
			function moveLeft(times, state){
				...
				$newitem5 = $item4;
				$newitem5.removeClass('frame4').addClass('frame5').stop().animate({ opacity:o.back_opacity, left:item5_pos, top:back_top }, zpeed, eazing)
					.find('img').stop().animate({ width:back_img_width, height:back_img_height, marginTop:back_margin.top, marginRight:back_margin.right, marginBottom:back_margin.bottom, marginLeft:back_margin.left, opacity:1 }, zpeed, eazing)
					.end().find('h6:visible').stop().animate({ fontSize:back_header }, zpeed, eazing);
				...
				$newitem4 = $item3;
				$newitem4.removeClass('frame3').addClass('frame4').stop().animate({ opacity:o.behind_opacity, left:item4_pos, top:behind_top }, zpeed, eazing)
					.find('img').stop().animate({ width:behind_img_width, height:behind_img_height, marginTop:behind_margin.top, marginRight:behind_margin.right, marginBottom:behind_margin.bottom, marginLeft:behind_margin.left, opacity:1 }, zpeed, eazing)
					.end().find('h6:visible').stop().animate({ fontSize:behind_header }, zpeed, eazing);
				if(ie6){ $newitem4.find('span:visible').stop().animate(ie6_behind_span_animate, zpeed, eazing); }
				else{ $newitem4.find('span').stop().animate(behind_span_animate, zpeed, eazing); }
				...
				$newitem2 = $item1;
				$newitem2.removeClass('frame1').addClass('frame2').stop().animate({ opacity:o.behind_opacity, left:item2_pos, top:behind_top }, zpeed, eazing)
					.find('img').stop().animate({width:behind_img_width, height:behind_img_height, marginTop:behind_margin.top, marginRight:behind_margin.right, marginBottom:behind_margin.bottom, marginLeft:behind_margin.left, opacity:1 }, zpeed, eazing)
					.end().find('h6:visible').stop().animate({ fontSize:behind_header }, zpeed, eazing);
				if(ie6){ $newitem2.find('span:visible').stop().animate(ie6_behind_span_animate, zpeed, eazing); }
				else{ $newitem2.find('span').stop().animate(behind_span_animate, zpeed, eazing); }
				...
				$item5 = $newitem5;
				// New frame description
				if(ie6){ $item1.find('span:visible').css(ie6_back_span_animate); }
				else{ $item1.find('span').css(back_span_animate); }
				// Remove the out of range item
				$container.children('.remove').fadeOut(zpeed, function(){ $(this).remove(); });
				// Set new current
				if(current === 1){
...
// ACTIONS
			...
			$container.delegate('.frame1', 'click', function(){
			...
			$container.delegate('.frame2', 'click', function(){
				if(o.freescroll || !busy){
					moveLeft(1);
					return false;
				}
			...
			$container.delegate('.frame4', 'click', function(){
				if(o.freescroll || !busy){
					moveRight(1);
					return false;
				}
			...
			$container.delegate('.frame5', 'click', function(){
				if(o.freescroll || !busy){
					if(o.never_move_twice){
						moveRight(1);
					}else if(o.move_more_directly){
            ...
            */
			...
			$container.delegate('.frame3', 'hover', function(e){
				if((e.type === 'mouseover' || e.type === 'mouseenter') && !busy){
					if(o.autoplay){
						stopInterval();
					}
					// Var image height
					var animateframe = { left:'-='+(o.front_img_width*(o.hovergrowth/2)), top:'-='+(o.front_img_width*(o.hovergrowth)) };
					if(!var_heights){
						animateframe.top = '-='+(o.front_img_height*o.hovergrowth);
					}
					var animateimage = { width:(o.front_img_width*(1+o.hovergrowth)) };
					if(!var_heights){
						animateimage.height = (o.front_img_height*(1+o.hovergrowth));
					}

					$item3.addClass('zoomed')
						.stop(true,true).animate(animateframe, hoverspeed)
						.find('img').stop().animate(animateimage, hoverspeed);
