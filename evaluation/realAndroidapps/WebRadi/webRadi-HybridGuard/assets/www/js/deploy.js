(function() {
	...
	var configFilename = 'wr-config.json';
	...
	if (!window.phonegap.app.configFilename) window.phonegap.app.configFilename = configFilename;
    ...
    window.phonegap.app.config.load = function(callback) {
        readFile(configFilename, function(e, text) {
            config = parseAsJSON(text);

            // load defaults
            config.emulatorAddress = config.emulatorAddress || '127.0.0.1:3000';
			config.notificationsSenderId = config.notificationsSenderId || '';
            callback(config);
        });
    };
    ...
    window.phonegap.app.config.save = function(data, callback) {
        saveFile(configFilename, data, function(e) {
            callback();
        });
    };
    ...
    function readFile(filepath, callback) {
		window.resolveLocalFileSystemURL(cordova.file.dataDirectory + filepath, function(fileEntry) {
			fileEntry.file(function gotFile(file) {
				var reader = new FileReader();
				reader.onloadend = function(evt) {
					// #72 - Fix WP8 loading of config.json
					// On WP8, `evt.target.result` is returned as an object
					// instead
					// of a string. Since WP8 is using a newer version of
					// the File API
